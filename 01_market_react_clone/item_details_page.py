                        ...
                        Div(cls='flex-1 space-y-6 px-0 sm:space-y-8 md:px-4 lg:px-11')(
                            ...
                            Div(cls='w-full flex flex-col space-y-4')(
                                ...
                                Div(cls='space-y-2 relative')(
                                    ...
                                    Form(action="/download", method="POST", cls="w-full")(
                                        Input(type="hidden", name="url", value=item.file_path),
                                        Div(cls='relative rounded-md')(
                                            Input(id='credentials', placeholder='Enter credentials', aria_describedby='credentials-error', type='text', name='credentials', cls='block w-full rounded-md border-0 px-2 py-1.5 text-black ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-1 focus:ring-inset focus:ring-zinc-950 shadow-none outline-none sm:text-sm sm:leading-6')
                                        ),
                                        Button('Download File', 
                                               type='submit', 
                                               cls='mt-4 flex transition-all w-full justify-center rounded-lg px-3 py-2 text-base text-white outline-none hover:bg-violet-800 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 bg-violet-700 shadow-inner-white outline-none ring-1 ring-inset ring-zinc-700'),
                                    )
                                )
                            )
                        )
